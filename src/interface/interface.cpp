...
interface::interface(std::istream& input, std::ostream& output, std::ostream& error)
    ...
    this->output << "|" << std::endl
                 << "? Attempting to load Department Students, Professors and Courses from files..." << std::endl;
    this->error << "! ERROR: Files Not Found. Secretary will be left empty." << std::endl
                << "|" << std::endl;
}
